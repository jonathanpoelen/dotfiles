...
// Note: I (Jon) removed comment alignement and 1 preprocessor level
...
function tryStringLiteral(cursor, ch)
{
    ...
    if (column < 2) {
        ...
        if (!new_string_just_started && ch === '"') {
            ...
            var stringEnd;
            const isRawPrefix = (column >= 2 && document.charAt(line, column - 2) === 'R');
            if (isRawPrefix) {
                const charBeforeRaw = (column >= 3 && document.charAt(line, column - 3));
                if (column >= 4 && charBeforeRaw === '8' && document.charAt(line, column - 4) === 'u') {
                    raw_string = true;
                    stringEnd = 5;
                } else if (column >= 3 && (charBeforeRaw === 'u' || charBeforeRaw === 'U' || charBeforeRaw == 'L')) {
                    raw_string = true;
                    stringEnd = 4;
                } else {
                    raw_string = true;
